...
describe("client context coordinator", () => {
  ...
  it("uses full protocol first, short reminders later, and exposes every removable stable id", async () => {
    ...
    h.coordinator.enable();
    const turnHooks = h.registered.turn as { beforeTurn(event: typeof turn): Promise<{ promptContributions?: TestContribution[]; internalMessageCapture?: { nonce?: string; maxBytes: number; validate?: (payload: unknown) => boolean } }> };
  ...
  it("rekeys a pending first turn to its native session without repeating the full protocol", async () => {
    ...
    const identified = await turnHooks.beforeTurn({ ...turn, runId: "pending-run", turnId: "native-turn", sessionId: "native-session" });
  ...
  it("isolates concurrent pending sessions until each receives a native identity", async () => {
    ...
    const leftNative = await turnHooks.beforeTurn({ ...turn, runId: "left-run", turnId: "left-native", sessionId: "left-session" });
    const rightNative = await turnHooks.beforeTurn({ ...turn, runId: "right-run", turnId: "right-native", sessionId: "right-session" });
